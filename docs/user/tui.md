...
| `/skill [install \| status]` | Install/update the repo's shipped Claude skills (like `recalling-past-sessions`, which teaches any Claude session to search this archive) into `~/.claude/skills/`. Bare, reports each skill's state (missing/stale/current); `install` renders them for this checkout's location and writes them. Idempotent; re-run after `/update`. This is the one command that writes outside the archive, into your Claude config — that's its purpose. |
