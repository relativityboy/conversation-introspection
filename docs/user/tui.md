...
| `/skill [install \| status]` | Install/update the repo's shipped Claude skills into `~/.claude/skills/`: `recalling-past-sessions` (teaches any Claude session to search this archive) and `session-name` (`/session-name <name>` names the current session in the archive, importing it first if needed — see [Editable titles](reading-room.md#editable-titles)). Bare, reports each skill's state (missing/stale/current); `install` renders them for this checkout's location and writes them. Idempotent; re-run after `/update`. This is the one command that writes outside the archive, into your Claude config — that's its purpose. |
| `/exclude [add <path-or-slug> [reason...] \| remove <slug> \| list]` | Wall a project off from capture — for sensitive/classified work. An excluded project's directory is skipped by every import *before anything under it is read*. Exclude **before** the sensitive work starts (cron imports run every 15 minutes). Takes a filesystem path (auto-encoded to the CLI's slug) or a raw slug, plus an optional reason. Prevention only: already-captured sessions remain until deleted. Owner-only — no API path can exclude or reveal exclusions. |
| `/delete <uuid-or-slug> [yes [reason...] \| backups yes \| forbid yes]` | **Irreversible** deletion of a session or whole project — the repair tool for sensitive data that already landed. Bare, it previews (counts, warning, locations) and deletes nothing; only the explicit `yes` form deletes, writing a ledger row (target, time, your optional reason — the archive remembers *that* it forgot, never what). Two separate follow-up asks, never automatic: `backups yes` scrubs backup DB copies; `forbid yes` blocks re-import of a session whose source files still exist. Owner-only — no API can delete. |
