...
| [The TUI](tui.md) | The interactive terminal UI: searching the archive, and every slash command (`/import`, `/reparse`, `/export`, `/status`, `/unarchive`, `/start-web`, `/stop-web`, `/cron`, `/update`, `/restart`, `/help`, `/quit`) — including the public-bind warning. |
