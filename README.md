...
| `introspect session-name [<name> \| --stdin] [--session <uuid>] [--url <base>]` | Names a session in the archive — the one it runs inside by default (`CLAUDE_CODE_SESSION_ID`) — importing it first if the belt hasn't captured it | Talks to the running server; prints one line saying what happened (server not running / too old / named). This is what `/session-name` runs |
